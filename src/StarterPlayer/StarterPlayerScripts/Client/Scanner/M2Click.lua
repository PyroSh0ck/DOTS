...
function M2Click.ScaledStepSize(renderDistance, camera, viewportSize)
    ...
    local res1 = RaycastUtils.SendRaycast(ray1.Origin, ray1.Direction.Unit * renderDistance)
    local res2 = RaycastUtils.SendRaycast(ray2.Origin, ray2.Direction.Unit * renderDistance)
    local res3 = RaycastUtils.SendRaycast(ray3.Origin, ray3.Direction.Unit * renderDistance)
...
function M2Click.Click(scanner)
    ...
    if not scanner.scanning and not scanner.m2Cooldown then
        if scanner.scannable then
            ...
            local stepSizeVector = M2Click.ScaledStepSize(scanner.renderDistance, cam, viewportSize)
            ...
            if stepSizeVector then
                ...
                while (yVal + stepSizeVector.Y) < viewportSize.Y do
                    while (xVal + stepSizeVector.X) < viewportSize.X do
                        ...
                        local raycastResult = RaycastUtils.SendRaycast(ray.Origin, randomRay)
